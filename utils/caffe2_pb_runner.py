...
cmd_line_parser.add_argument('--image', '-i',
                             required=True,
                             help="Image to be processed by the neural network")
cmd_line_parser.add_argument('--directory', '-d',
                             required=True,
...
cmd_line_parser.add_argument('--model-name', '-m',
                             required=False,
...
def mode_0to256 (x): return x
def mode_0to1 (x): return x/255
def mode_128to127 (x): return x-128
...
if MODEL not in list(model_props.keys()):
    print("Model " + MODEL + " is not supported. Specify --model-name (-m) if "
          "it is not the base name of the directory containing pb files.")
    exit(1)

MODEL_ROOT = args.directory
IMAGE_LOCATION = args.image
img = skimage.img_as_ubyte(skimage.io.imread(
    IMAGE_LOCATION)).astype(np.float32)
...
transposed_image = np.zeros((1, model_props[MODEL].num_color_channels, model_props[MODEL].image_size, model_props[MODEL].image_size)).astype(np.float32)
for w in range(0, model_props[MODEL].image_size):
    for h in range (0, model_props[MODEL].image_size):
        for c in range(0, model_props[MODEL].num_color_channels):
            # WHC -> CWH, RGB -> BGR
            transposed_image[0][model_props[MODEL].num_color_channels-c-1][w][h] = model_props[MODEL].image_mode_op(img[w][h][c])
...
print("Predicted class at index {} with probability {}".format(max_idx, results[0][0][max_idx]))

...
Error PyTorchModelLoader::loadGelu(const torch::jit::Node *ptNode) {
  ...
  if (inputs.size() == 2) {
    ...
    int64_t approximate;
    ASSIGN_VALUE_OR_RETURN_ERR(approximate, iValToInt(getGlowIValueForValue(inputs[1])));
    if (approximate != 0) {
      return MAKE_ERR(strFormat(
            "GELU approximation not supported; found value: %ld", approximate));
    }
  } else if (inputs.size() > 2) {
    return MAKE_ERR(strFormat(
          "Expected at most 2 inputs for GELU, but found: %d", int(inputs.size())));
  }

...
struct InferenceIO
{
    ...
    void *output;
    /// Should input memory be deallocated (yes if we allocated it, no if we were passed 
    /// a pointer to a previously allocated region)
    int cleanup_input;
    /// Should output memory be deallocated (yes if we allocated it, no if we were passed 
    /// a pointer to a previously allocated region)
    int cleanup_output;
    /// Input data length
    size_t in_len;
...
/// Initialize runtime data \p runtime_data given the network metadata \p network_data.
...
/// Initialize inference IO metadata given pointers to the IO memory \p in_mmap and \p out_mmap.
/// \p in_mmap may be null, then memory is allocated (and deallocated when done).
/// \p out_mmap may be null, then memory is allocated (and deallocated when done).
/// If \p in_mmap and \p out_mmap are not null, IO memory is not allocated/deallocated.
/// returns X_SUCCESS on success, X_FAILURE on failure.
int init_io(struct InferenceIO *iio, void *in_mmap, void *out_mmap);

/// Clean up the runtime data \p runtime_data, which ammounts to deallocating allocated resources.
void cleanup_runtime_data(struct RuntimeData *runtime_data);

/// Clean up inference IO \p io, which ammounts to deallocating allocated resources.
void cleanup_io(struct InferenceIO *io);

/// Run the inference given Inference IO \p iio and the runtime data \p runtime_data.

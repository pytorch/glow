...
// If performance monitoring is enabled, make sure to correctly clear/initialize the corresponding
// runtime data.
#ifdef ENABLE_PERF_MONITORING
...
// If performance monitoring is enabled, report total weights size as part of the monitoring.
#ifdef ENABLE_PERF_MONITORING
        if (runtime_data->do_perf_monitoring) {
...
int init_io(struct InferenceIO *iio, void *in_mmap, void *out_mmap)
{
    ...
    // This will return X_FAILURE. Success is returned if and only if both input and output
    // are not null (either memory mapped, or successfully allocated)
    return retval;
}
...
uint8_t *init_constant_weights(const char *wfname, const struct BundleConfig *bundle_config)
{
    size_t size = 0;
    int fd = 0;
    off_t file_offset = 0;
...
/// \p network_data - Pointer to the NetworkData structure holding metadata for the network.
/// \p in_offset - Output for the input offset (pointer of type size_t*)
/// \p out_offset - Output for the output offset (pointer of type size_t*)
/// \returns X_SUCCESS on success, X_FAILURE on failure (in_offset and out_offset are not valid)
int get_io_offsets(const struct NetworkData *network_data, size_t *in_offset, size_t *out_offset)
{
    int retval;
    bool found_in = false;
    ...
    // Look for the input and the output tensors, and grab their offsets. The lookup is done 
    // by tensor name.

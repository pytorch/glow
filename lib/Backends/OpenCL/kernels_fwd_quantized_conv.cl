...
__kernel
  ...
  // Initialize the accumulation registers.
  {
    ...
    float matMulScale = b_scale * a_scale;
// Initialize the accumulation registers.
#pragma unroll
    for (int_tp wm = 0; wm < WPTM; ++wm) {
#pragma unroll
      for (int_tp wn = 0; wn < WPTN / VWN; ++wn) {
        VEC_4_0(Creg[wm][wn]) = c_offset;
        VEC_4_1(Creg[wm][wn]) = c_offset;
        VEC_4_2(Creg[wm][wn]) = c_offset;
        VEC_4_3(Creg[wm][wn]) = c_offset;
      }
    }
    {
// Loop over all tiles.
...
#pragma unroll 4
        ...
        // Use int4 here to avoid overflow.
        int4 Areg;
        int4 Breg[WPTN / VWN];
...
#pragma unroll
            for (int_tp wn = 0; wn < WPTN / VWN; ++wn) {
              ...
              VEC_4_0(Breg[wn]) = B_OFFSET(Bsub[k][col + 0 * RTSN]);
              VEC_4_1(Breg[wn]) = B_OFFSET(Bsub[k][col + 1 * RTSN]);
              VEC_4_2(Breg[wn]) = B_OFFSET(Bsub[k][col + 2 * RTSN]);
              VEC_4_3(Breg[wn]) = B_OFFSET(Bsub[k][col + 3 * RTSN]);
            }
// Perform the computation.
#pragma unroll
            for (int_tp wm = 0; wm < WPTM / VWM; ++wm) {
              int_tp row = tidm + wm * VWM * RTSM;
              VEC_4_0(Areg) = A_OFFSET(Asub[row + 0 * RTSM][k]);
              VEC_4_1(Areg) = A_OFFSET(Asub[row + 1 * RTSM][k]);
              VEC_4_2(Areg) = A_OFFSET(Asub[row + 2 * RTSM][k]);

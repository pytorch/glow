...
void libjit_resizenearest_generic(DstTy *dst, const SrcTy *src,
                                  const float *scale, const size_t *inWdims,
                                  const size_t *outWdims) {

  for (size_t ob = 0; ob < outWdims[0]; ++ob) {
    auto ib = std::min(size_t(ob / (scale[0])), inWdims[0] - 1);
    for (size_t oh = 0; oh < outWdims[1]; ++oh) {
      auto ih = std::min(size_t(oh / (scale[1])), inWdims[1] - 1);
      for (size_t ow = 0; ow < outWdims[2]; ++ow) {
        auto iw = std::min(size_t(ow / (scale[2])), inWdims[2] - 1);
        for (size_t oc = 0; oc < outWdims[3]; ++oc) {
          auto ic = std::min(size_t(oc / (scale[3])), inWdims[3] - 1);
          const size_t inIndex = libjit_getXYZW(inWdims, ib, ih, iw, ic);
          const size_t outIndex = libjit_getXYZW(outWdims, ob, oh, ow, oc);

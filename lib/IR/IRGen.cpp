...
Value *IRGenVisitor::valueForNode(NodeValue N) {
  // For FusionGroupPlaceholderNode, use corresponding input value.
  if (auto *V = dyn_cast<FusionGroupPlaceholderNode>(N)) {
    N = V->getParentFusionGroup()->getNthInput(V->getFusionGroupInputIndex());
  }
  // For FusionGroupNode, use the corresponding subgraph output.
  while (auto *V = dyn_cast<FusionGroupNode>(N.getNode())) {
    N = V->getGraphOutputs()[N.getResNo()];
  }
...
#include "glow/AutoGenIRGen.h"
  ...
  case glow::Kinded::Kind::FusionGroupNodeKind: {

    auto *FGN = cast<FusionGroupNode>(N);

    // The visior implementation of FusionGroupNode guarantees
    // its subgraph to be visited before post visit.
    // IRGenVisior should gather all generated IR for the subgraph
    // (in scheduled order) and create FusionGroupInst out of them.

    auto subgraph = FGN->getFusionGraph();
    llvm::SmallVector<Instruction *, 4> subgraphInst;
    ...
    // Push is required since FusionGroupInst is not generated by Autogen.
    F_->pushInstr(FGI);

    break;
  }

  case glow::Kinded::Kind::FusionGroupPlaceholderNodeKind: {
    // Skip IRGen for FusionGroupPlaceholder.
    // The original value will be used in IRGenVisitor::valueForNode
    break;
  }
...
void IRFunction::generateIR(const Backend &B) {
  ...
  for (auto &N : ScheduledNodes) {
    // Only start visiting nodes that is not fused.
    // Fused subgraph will be visited from FusionGroupNode.
    // TODO: Support Storage nodes with FusionGroup.
    if (!N->isFused()) {

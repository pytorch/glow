...
void Function::addGradientVariable(Variable *V, Variable *GradV) {
  advanceState(State::Differentiated);
  grads_.push_back({V, GradV});
}

Variable *Function::getGradientVariable(Variable *V) {
  for (auto &p : grads_) {
    if (p.first == V) {
      return p.second;
    }
...
ConvolutionNode *Function::createConv(llvm::StringRef name, NodeValue input,
  ...
  auto *filter = M_->createVariable(ElemKind::FloatTy, filterDim, "filter",
                                Variable::VisibilityKind::Private,
                                Variable::TrainKind::Xavier, fanIn);

  auto *bias = M_->createVariable(ElemKind::FloatTy, {depth}, "bias",
                              Variable::VisibilityKind::Private,
...
FullyConnectedNode *Function::createFullyConnected(llvm::StringRef name,
  ...
  TypeRef OT = M_->getVoidTy();

  // if \p input is of type void, we cannot calculate the dimensions
  if (T != M_->getVoidTy()) {
    OT = M_->uniqueTypeWithNewShape(T, {input.dims()[0], outDepth});
  }
...
FullyConnectedNode *Function::createFullyConnected(llvm::StringRef name,
  ...
  auto *W = M_->createVariable(T->getElementType(), {idim.second, outDepth},
                           "weights", Variable::VisibilityKind::Private,
                           Variable::TrainKind::Xavier, fanIn);

  auto *B = M_->createVariable(T->getElementType(), {outDepth}, "bias",
                           Variable::VisibilityKind::Private,
                           Variable::TrainKind::Broadcast, 0.1);
...
RegressionNode *Function::createRegression(llvm::StringRef name, NodeValue input,
                                        NodeValue expected) {
...
ReshapeNode *Function::createReshape(llvm::StringRef name, NodeValue input,
  ...
  auto TR = M_->uniqueTypeWithNewShape(input.getType(), shape);
...
TransposeNode *Function::createTranspose(llvm::StringRef name, NodeValue input,
  ...
  auto NT = M_->uniqueTypeWithNewShape(input.getType(), shape);
...
BroadcastNode *Function::createBroadcast(llvm::StringRef name, NodeValue input,
                                      llvm::ArrayRef<size_t> shape,
                                      unsigned axis) {
  auto TR = M_->uniqueType(input.getType()->getElementType(), shape);
...
BatchNormalizationNode *Function::createBatchNormalization(llvm::StringRef name,
  ...
  auto *beta = M_->createVariable(ElemKind::FloatTy, {channels}, "beta",
                              Variable::VisibilityKind::Private,
                              Variable::TrainKind::Broadcast, 0.);
  auto *gamma = M_->createVariable(ElemKind::FloatTy, {channels}, "gamma",
                               Variable::VisibilityKind::Private,
                               Variable::TrainKind::Broadcast, 1.0);

  auto *mean = M_->createVariable(ElemKind::FloatTy, {channels}, "mean",
                              Variable::VisibilityKind::Private,
                              Variable::TrainKind::None);
  auto *variance = M_->createVariable(ElemKind::FloatTy, {channels}, "variance",
                                  Variable::VisibilityKind::Private,
                                  Variable::TrainKind::None);
...
Function::createLocalResponseNormalization(llvm::StringRef name, NodeValue input,
...
SplatNode *Function::createSplat(llvm::StringRef name, TypeRef ty, float value) {
...
BatchedMatMulNode *Function::createBatchedMatMul(llvm::StringRef name,
                                              ...
                                              NodeValue rhs) {
  return addNode(new BatchedMatMulNode(name, M_->uniqueType(*outTy), lhs, rhs));
}
...
BatchedMatMulNode *Function::createBatchedMatMul(llvm::StringRef name,
  ...
  auto ty = M_->uniqueTypeWithNewShape(lhs.getType(), {N, X, Y});
...
BatchedReduceNode *Function::createBatchedReduce(llvm::StringRef name,
  ...
  return addNode(new BatchedReduceNode(name, M_->uniqueType(RT), mode, batch));
}
...
SaveNode *Function::createSave(llvm::StringRef name, NodeValue input) {
  auto *dest =
      M_->createVariable(input.getType(), name, Variable::VisibilityKind::Private,
                     Variable::TrainKind::None);
...
QuantizationProfileNode *Function::createQuantizationProfile(llvm::StringRef name,
                                                          NodeValue input) {
  // TODO: this size is going to be refined. Just a placeholder now.
  const size_t numberOfBuckets = 2000U;
  auto *histogram = M_->createVariable(
  ...
  auto *computationInfo = M_->createVariable(
...
TopKNode *Function::createTopK(llvm::StringRef name, NodeValue input, size_t k) {
  ...
  outDims.back() = k;
  return addNode(
      new TopKNode(name, M_->uniqueType(input->getElementType(), outDims),
                   M_->uniqueType(ElemKind::IndexTy, outDims), input, k));
}
...
GatherNode *Function::createGather(llvm::StringRef name, NodeValue data,
  ...
  return addNode(new GatherNode(
      name, M_->uniqueType(data->getElementType(), outDims), data, indices));
}
...
DequantizeNode *Function::createDequantize(llvm::StringRef name, NodeValue input) {
  assert(input.getElementType() == ElemKind::Int8QTy &&
         "Input must be a quantized type");
  TypeRef outTy = M_->uniqueType(Type(ElemKind::FloatTy, input.dims()));
...
void Function::createSimpleRNN(llvm::StringRef namePrefix,
  ...
  auto *HInit = M_->createVariable(ElemKind::FloatTy, {batchSize, hiddenSize},
  ...
  float b = 0.1;
  auto *Whh = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             (namePrefix + ".Whh").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bhh = M_->createVariable(
      ElemKind::FloatTy, {hiddenSize}, (namePrefix + ".Bhh").str(),
      Variable::VisibilityKind::Private, Variable::TrainKind::Broadcast, b);
  auto *Wxh = M_->createVariable(ElemKind::FloatTy, {inputSize, hiddenSize},
                             (namePrefix + ".Wxh").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, inputSize);
  auto *Bxh = M_->createVariable(
      ElemKind::FloatTy, {hiddenSize}, (namePrefix + ".Bxh").str(),
      Variable::VisibilityKind::Private, Variable::TrainKind::Broadcast, b);
  auto *Why = M_->createVariable(ElemKind::FloatTy, {hiddenSize, outputSize},
                             (namePrefix + ".Why").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bhy = M_->createVariable(
...
void Function::createGRU(llvm::StringRef namePrefix, llvm::ArrayRef<Node *> inputs,
  ...
  // update gate
  float bUpdate = 0.1;
  auto *Wxz = M_->createVariable(ElemKind::FloatTy, {inputSize, hiddenSize},
                             (namePrefix + ".Wxz").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, inputSize);
  auto *Whz = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             (namePrefix + ".Whz").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bz1 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bz1").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bUpdate);
  auto *Bz2 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bz2").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bUpdate);
  float bReset = -1.0;
  // reset gate
  auto *Wxr = M_->createVariable(ElemKind::FloatTy, {inputSize, hiddenSize},
                             (namePrefix + ".Wxr").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, inputSize);
  auto *Whr = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             (namePrefix + ".Whr").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Br1 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".br1").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bReset);
  auto *Br2 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".br2").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bReset);
  ...
  auto *Whh = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             ...
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bh1 = M_->createVariable(
      ElemKind::FloatTy, {hiddenSize}, (namePrefix + ".bh1").str(),
      Variable::VisibilityKind::Private, Variable::TrainKind::Broadcast, b);
  auto *Bh2 = M_->createVariable(
  ...
  auto *Why = M_->createVariable(ElemKind::FloatTy, {hiddenSize, outputSize},
                             ...
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *By = M_->createVariable(
      ElemKind::FloatTy, {outputSize}, (namePrefix + ".by").str(),
      Variable::VisibilityKind::Private, Variable::TrainKind::Broadcast, b);

  auto *Ones = M_->createVariable(
...
void Function::createLSTM(llvm::StringRef namePrefix,
  ...
  // forget gate
  float bForget = 1.0;
  auto *Wxf = M_->createVariable(ElemKind::FloatTy, {inputSize, hiddenSize},
                             (namePrefix + ".Wxf").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, inputSize);
  auto *Whf = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             (namePrefix + ".Whf").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bf1 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bf1").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bForget);
  auto *Bf2 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bf2").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bForget);
  // input gate
  float bInput = 0.1;
  auto *Wxi = M_->createVariable(ElemKind::FloatTy, {inputSize, hiddenSize},
                             (namePrefix + ".Wxi").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, inputSize);
  auto *Whi = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             (namePrefix + ".Whi").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bi1 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bi1").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bInput);
  auto *Bi2 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bi2").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bInput);
  ...
  // output gate
  float bOutput = 0.1;
  auto *Wxo = M_->createVariable(ElemKind::FloatTy, {inputSize, hiddenSize},
                             (namePrefix + ".Wxo").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, inputSize);
  auto *Who = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             (namePrefix + ".Who").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bo1 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bo1").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bOutput);
  auto *Bo2 = M_->createVariable(ElemKind::FloatTy, {hiddenSize},
                             (namePrefix + ".bo2").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Broadcast, bOutput);
  ...
  // cell state
  float bCell = 0.1;
  auto *Wxc = M_->createVariable(ElemKind::FloatTy, {inputSize, hiddenSize},
                             (namePrefix + ".Wxc").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, inputSize);
  auto *Whc = M_->createVariable(ElemKind::FloatTy, {hiddenSize, hiddenSize},
                             (namePrefix + ".Whc").str(),
                             Variable::VisibilityKind::Private,
                             Variable::TrainKind::Xavier, hiddenSize);
  auto *Bc1 = M_->createVariable(
      ElemKind::FloatTy, {hiddenSize}, (namePrefix + ".bc1").str(),
      Variable::VisibilityKind::Private, Variable::TrainKind::Broadcast, bCell);
  auto *Bc2 = M_->createVariable(
...
//===----------------------------------------------------------------------===//
//                   Function dumping and printing
//===----------------------------------------------------------------------===//

void Function::dump() const {
  llvm::outs() << "Function structure " << getName() << ":\n";
...
/// A helper class for visiting and generating the dotty file from the function.
...
void Function::verify() const {
  ...
  // Any node referenced by one of the function's nodes should either belong
  // to the function, or be a variable (belonging to module).
  for (auto *N : nodes_) {
    for (size_t idx = 0, e = N->getNumInputs(); idx < e; ++idx) {
      assert((std::find(nodes_.begin(), nodes_.end(), N->getNthInput(idx)) !=
                  nodes_.end() ||
              std::find(M->getVars().begin(), M->getVars().end(), N->getNthInput(idx)) !=
                  M->getVars().end()) &&
             "Every node referenced by one of the function"
             " nodes should be part of the function");
    }
  }
...
void Function::resetState() { state_ = State::Created; }

void Function::advanceState(State s) {
  assert(state_ <= s && "Wrong order of actions with a function.");
  state_ = s;
}

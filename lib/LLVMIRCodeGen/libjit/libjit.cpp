...
static void libjit_insert_tensor(ElemTy *tensor, ElemTy *slice, dim_t *offset,
                                 ...
                                 dim_t numDims, dim_t count, dim_t axis) {
          ...
          for (dim_t z = 0; z < sliceDim[2]; z++) {
            for (dim_t w = 0; w < sliceDim[3]; w++) {
              for (dim_t q = 0; q < sliceDim[4]; q++) {
                ...
                tensor[libjit_getXYZWQ(tensorDim, C[0], C[1], C[2], C[3], C[4])] = *slice++;
              }
            }
          }
        }
      }
      slice -= sliceDim[0] * sliceDim[1] * sliceDim[2] * sliceDim[3] * sliceDim[4];
    }
    return;
  }
        ...
        for (dim_t y = 0; y < sliceDim[1]; y++) {
          for (dim_t z = 0; z < sliceDim[2]; z++) {
            for (dim_t w = 0; w < sliceDim[3]; w++) {
              ...
              C[2] = z + offsets_cpy[2] + ((axis == 2) ? countAxisOffset : 0);
              C[3] = w + offsets_cpy[3] + ((axis == 3) ? countAxisOffset : 0);
              tensor[libjit_getXYZW(tensorDim, C[0], C[1], C[2], C[3])] = *slice++;
            }
          }
        }
      }
      slice -= sliceDim[0] * sliceDim[1] * sliceDim[2] * sliceDim[3];
    }
    return;
  }
      ...
      for (dim_t x = 0; x < sliceDim[0]; x++) {
        for (dim_t y = 0; y < sliceDim[1]; y++) {
          for (dim_t z = 0; z < sliceDim[2]; z++) {
            ...
            C[1] = y + offsets_cpy[1] + ((axis == 1) ? countAxisOffset : 0);
            C[2] = z + offsets_cpy[2] + ((axis == 2) ? countAxisOffset : 0);
            tensor[libjit_getXYZ(tensorDim, C[0], C[1], C[2])] = *slice++;
          }
        }
      }
      slice -= sliceDim[0] * sliceDim[1] * sliceDim[2];
    }
    return;
  }
    ...
    for (dim_t c = 0; c < count; c++) {
      for (dim_t x = 0; x < sliceDim[0]; x++) {
        for (dim_t y = 0; y < sliceDim[1]; y++) {
          ...
          C[0] = x + offsets_cpy[0] + ((axis == 0) ? countAxisOffset : 0);
          C[1] = y + offsets_cpy[1] + ((axis == 1) ? countAxisOffset : 0);
          tensor[libjit_getXY(tensorDim, C[0], C[1])] = *slice++;
        }
      }
      slice -= sliceDim[0] * sliceDim[1];
    }
    return;
  }
  ...
  if (numDims == 1) {
    for (dim_t c = 0; c < count; c++) {
      for (dim_t x = 0; x < sliceDim[0]; x++) {
        const dim_t countAxisOffset = c * sliceDim[axis];
        tensor[x + offsets_cpy[0] + ((axis == 0) ? countAxisOffset : 0)] = *slice++;
      }
      slice -= sliceDim[0];
    }
    return;
...
template <typename ElemTy>
static void libjit_extract_tensor(ElemTy *tensor, ElemTy *slice, dim_t *sliceDims, dim_t numDims,
                                  dim_t tensorStart, sdim_t *tensorOffsets) {

  tensor += tensorStart;

  if (numDims == 5) {
    for (dim_t x = 0; x < sliceDims[0]; x++) {
      for (dim_t y = 0; y < sliceDims[1]; y++) {
        for (dim_t z = 0; z < sliceDims[2]; z++) {
          for (dim_t w = 0; w < sliceDims[3]; w++) {
            for (dim_t q = 0; q < sliceDims[4]; q++) {
              *slice++ = *tensor;
...
void libjit_extract_tensor_f(float *tensor, float *slice, dim_t *sliceDims, dim_t numDims,
                             dim_t tensorStart, sdim_t *tensorOffsets) {
  libjit_extract_tensor(tensor, slice, sliceDims, numDims, tensorStart, tensorOffsets);
}

void libjit_extract_tensor_i8(int8_t *tensor, int8_t *slice, dim_t *sliceDims, dim_t numDims,
                              dim_t tensorStart, sdim_t *tensorOffsets) {
  libjit_extract_tensor(tensor, slice, sliceDims, numDims, tensorStart, tensorOffsets);
}

void libjit_extract_tensor_i32(int32_t *tensor, int32_t *slice, dim_t *sliceDims, dim_t numDims,
                               dim_t tensorStart, sdim_t *tensorOffsets) {
  libjit_extract_tensor(tensor, slice, sliceDims, numDims, tensorStart, tensorOffsets);
}

void libjit_extract_tensor_u(int64_t *tensor, int64_t *slice, dim_t *sliceDims, dim_t numDims,
                             dim_t tensorStart, sdim_t *tensorOffsets) {
  libjit_extract_tensor(tensor, slice, sliceDims, numDims, tensorStart, tensorOffsets);
}

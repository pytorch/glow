...
void lowerFullyConnectedNode(Function &graph, FullyConnectedNode &FC) {
  ...
  if (W->getType()->isQuantizedType()) {
    ...
    outTy = graph.getGraph()->uniqueType(elemTy, {1, xDim.first, wDim[1]}, FCT->getScale(),
                             FCT->getOffset());
  } else {
    outTy = graph.getGraph()->uniqueType(elemTy, {1, xDim.first, wDim[1]});
  }
...
void glow::lower(Function &G, CompilationMode mode) {
  G.advanceState(Function::State::Lowered);

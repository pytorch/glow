...
public:
  ...
  /// Insert \p T as the mapped Tensor for the Placeholder \p placeholder in the
  /// bindings of the context for \p node. This should not be called at the same
  /// time as getUniqueNodeContextPtr().
  void insertIntoNodePHBinding(const DAGNode *node,
                               const Placeholder *placeholder, Tensor &&T);

  /// \returns a unique pointer to an input bindings for \p node. This should
  /// not be called at the same time as insertIntoNodePHBinding().
  ...
  /// Insert \p T as the mapped Tensor for the Placeholder \p placeholder in the
  /// bindings of the result context. This should not be called at the same time
  /// as getUniqueResultPlaceholderBindingsPtr().
  void insertIntoResultPHBindings(const Placeholder *placeholder, Tensor &&T);

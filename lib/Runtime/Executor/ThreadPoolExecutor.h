...
public:
  ...
  /// Insert \p T as the mapped Tensor for the Placeholder with id \p id in the
  /// bindings of the context for \p node. This should not be called at the same
  /// time as getUniqueNodeContextPtr().
  void insertIntoNodeCtx(const DAGNode *node, uint32_t id, Tensor &&T);
  ...
  /// the result context with id \p id. This should not be called at the same
  /// time as getUniqueResultPlaceholderBindingsPtr().
  void insertIntoResultCtx(uint32_t id, Tensor &&T);
...
private:
  /// Create a Placeholder with id \p placeholderId and type \p type and store
  /// it in intermediatePlaceholders_. If a Placeholder already exists, return
  /// that.
  Placeholder *createOrGetPlaceholder(llvm::StringRef name,
  ...
  std::unordered_map<uint32_t, std::unique_ptr<Placeholder>>

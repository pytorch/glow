...
/// This example loads and runs the compiled vgg16 network model.
...
extern "C" void vgg16(uint8_t *constantWeightVars, uint8_t *mutableWeightVars,
                      uint8_t *activations);
extern "C" BundleConfig vgg16_config;
...
int main(int argc, char **argv) {
  ...
  uint8_t *constantWeightVarsAddr =
      initConstantWeights("vgg16.weights", vgg16_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(vgg16_config);
  uint8_t *activationsAddr = initActivations(vgg16_config);

  // Perform the computation.
  vgg16(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);

  // Report the results.
  dumpInferenceResults(vgg16_config, mutableWeightVarsAddr);

...
void testMNIST() {
  ...
  for (int epoch = 0; epoch < 60; epoch++) {
    ...
    for (int i = 0; i < numIterations; i++) {
      size_t slc = sampleCounter % numImages;
      ctx.get(A)->copyConsecutiveSlices(&imageInputs, slc);
      ctx.get(selected)->copyConsecutiveSlices(&labelInputs, slc);
      EE.run();
      sampleCounter++;
    }
  ...
  for (int iter = numIterations; iter < numIterations + 10; iter++) {
    Tensor *sample = ctx.get(A);
    sample->copyConsecutiveSlices(&imageInputs, minibatchSize * iter);
    EE.run();

    auto *resPH = llvm::cast<Placeholder>(result->getOutput().getNode());
    Tensor *res = ctx.get(resPH);

...
int main(int argc, char **argv) {
  ...
  auto inputAndOutput = loadResnet50Model(inputType, module.get(), 0);
  ...
  while (started++ < maxImages) {
    ...
    updateInputPlaceholders(*(context->getPlaceholderBindings()),
                            {inputAndOutput.first}, {&batch});

    dispatchClassify(0, hostManager.get(), inputAndOutput.second,
                     std::move(path), std::move(context), returned, finished);

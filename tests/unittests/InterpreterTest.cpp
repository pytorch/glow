...
TEST(Interpreter, trainASimpleNetwork) {
  ...
  inputs.getHandle<glow::DefaultFloatTy>() = {0.15, 0.15, 0.15, 0.15};
  expected.getHandle<glow::DefaultFloatTy>() = {0.9, 0.9, 0.9, 0.9};
  ...
  EE.infer({A}, {&inputs});
  auto RNWH = result->getOutput()->getPayload().getHandle<glow::DefaultFloatTy>();
  (void)RNWH;
...
TEST(Interpreter, simpleRegression) {
  ...
  auto I = inputs.getHandle<glow::DefaultFloatTy>();
  ...
  for (int iter = 0; iter < 5; iter++) {
    ...
    auto resH = result->getOutput()->getPayload().getHandle<glow::DefaultFloatTy>();
    (void)resH;
...
TEST(Interpreter, learnXor) {
  ...
  auto TS = trainingSet.getHandle<glow::DefaultFloatTy>();
  auto TL = trainingLabels.getHandle<glow::DefaultFloatTy>();
  auto TT = testingSet.getHandle<glow::DefaultFloatTy>();
...
void generateCircleData(Tensor &coordinates, Tensor &labels) {
  auto C = coordinates.getHandle<glow::DefaultFloatTy>();
...
TEST(Network, circle) {
  ...
  for (int x = -10; x < 10; x++) {
    for (int y = -10; y < 10; y++) {
      ...
      sample.getHandle<glow::DefaultFloatTy>() = {float(x) / 10, float(y) / 10};

      EE.infer({A}, {&sample});

      auto SMH = result->getOutput()->getPayload().getHandle<glow::DefaultFloatTy>();
      auto A = SMH.at({0, 0});
      auto B = SMH.at({0, 1});
...

  {
    ...
    Tensor sample(ElemKind::FloatTy, {1, 2});
    sample.getHandle<glow::DefaultFloatTy>() = {0., 0.};
    EE.infer({A}, {&sample});
    auto SMH = result->getOutput()->getPayload().getHandle<glow::DefaultFloatTy>();
    auto A = SMH.at({0, 0});
    auto B = SMH.at({0, 1});
...

  {
    ...
    Tensor sample(ElemKind::FloatTy, {1, 2});
    sample.getHandle<glow::DefaultFloatTy>() = {1., 1.};
    EE.infer({A}, {&sample});
    auto SMH = result->getOutput()->getPayload().getHandle<glow::DefaultFloatTy>();
    auto A = SMH.at({0, 0});
    auto B = SMH.at({0, 1});
...
TEST(Network, learnSingleValueConcat) {
  ...
  inputs.getHandle<glow::DefaultFloatTy>().clear(0.15);
  expected.getHandle<glow::DefaultFloatTy>().clear(0.9);

...
llvm::cl::OptionCategory EmbeddingBagBenchCat("EmbeddingBagBench Category");
llvm::cl::opt<bool> dumpOnnx("dump_onnx",
                             llvm::cl::desc("dump onnx text format for model"),
                             llvm::cl::Optional, llvm::cl::init(false),
                             llvm::cl::cat(EmbeddingBagBenchCat));
...
std::string getEBDescription(EBParam param) {
  std::string EBStr = std::string("EB");

  return strFormat(
      "%s__%zu_%zu__%zu__%zu__%zu", EBStr.c_str(),
...
class EBBench : public Benchmark {
  ...
  std::vector<EBParam> params_;
  bool convertFusedToFP32_;
  std::string devId_;

public:
  EBBench(dim_t batchSize_, dim_t asyncLaunchSize_, std::string backendStr_,
          std::vector<EBParam> params_, bool convertFusedToFP32,
  ...
  double countEBGbytes(EBParam param) const {
    ...
    if ((param.slsKind == NONQUANTIZED_WEIGHTED) ||
      ...
      input_gbytes += (param.numEBNodes * batchSize_ * avgIndicesPerBatch *
    ...
    // + indices
    input_gbytes +=
        (param.numEBNodes * batchSize_ * avgIndicesPerBatch * sizeof(int32_t)) /
        1e9;
    ...
    if ((param.slsKind == QUANTIZED_WEIGHTED) ||
      ...
      input_gbytes +=
          (param.numEBNodes * batchSize_ * avgIndicesPerBatch * elementSize) /
          1e9;
    }

    // + offsets
    input_gbytes += (param.numEBNodes * batchSize_ * sizeof(int32_t)) / 1e9;

    double output_gbytes = (param.numEBNodes * batchSize_ *
                            (param.numElementsPerRow * elementSize)) /
                           1e9;
  ...
  void addEBNode(std::unique_ptr<Module> &mod, Function *fn, EBParam param) {
    ...
    Constant *dataConstant = mod->createConstant("EBData", dataConstantTensor);
    ...
    auto *offsets =
        mod->createPlaceholder(ElemKind::Int32ITy, {batchSize_ + 1}, "offsets",
                               /* isTrainable */ false);
    ...
    for (dim_t i = 0; i < asyncLaunchSize_; i++) {
      ...
      auto offsetsHandle = contexts_[i]
                               ->getPlaceholderBindings()
                               ->allocate(offsets)
                               ->getHandle<int32_t>();
    ...
    R = fn->createEmbeddingBagByteRowwiseOffsets(
        getEBDescription(param), dataConstant, weights, indices, offsets,
        param.useFP16Accumulation, /*hasEndOffset*/ true, LM, avgLength);
  ...
  void setup() override {
    ...
    for (auto &param : params_) {
      for (dim_t i = 0; i < param.numEBNodes; i++) {
        addEBNode(mod, fn, param);
      }
    }

    fn->dumpDAG("ebbench.dot");
...
EBParam parseArgs(int argc, char *argv[]) {
  EBParam param;
  ...
  ASSIGN_VALUE_OR_FATAL(param.numEBNodes, getIntFromStr(argv[8]));
  ...
  printf("numEBNodes %zu\n", (size_t)param.numEBNodes);
  printf("slsKind %s\n", argv[9]);
  if (std::string(argv[9]) == "NonquantizedUnweighted") {
    ...
    llvm_unreachable("Invalid EB Kind");
  }
  printf("sortedStr %s\n", argv[10]);
...
int main(int argc, char *argv[]) {
  ...
  printf("EB Microbenchmark\n");
  printf("Usage: EBBench batchSize(Int) "
         ...
         "numAsyncLaunches(Int) numEBNodes(Int) "
  ...
  if (argc == 2) {
    ...
    while (getline(fin, line)) {
      ...
      EBParam param = parseArgs(argVec.size(), argVec.data());
      params.push_back(param);
      runHeader = std::string("_,benchName,_,filename");
      runPrefix = std::string(strFormat("EBBench,SW,%s", fname.c_str()));
    }
  }
  // Using command line
  else if (argc == 14 || argc == 15 || argc == 16 || argc == 17 || argc == 18) {
    EBParam param = parseArgs(argc, argv);
    params.push_back(param);
